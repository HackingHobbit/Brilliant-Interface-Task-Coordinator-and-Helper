...
function LoadingDots() {
  return (
    <div className="loading-dots" style={{ display: 'none' }}>
      {/* Removed large blinking white dots animation */}
    </div>
  );
}
    ...
    <>
      <div className="fixed top-0 left-0 right-0 bottom-0 z-50 flex justify-between p-4 flex-col pointer-events-none">
        ...
        <div className={`self-start p-6 rounded-lg max-w-none ${theme === 'futuristic' ? 'glass-panel scan-lines' : 'backdrop-blur-md bg-white bg-opacity-50'}`}>
          ...
          {currentIdentity && (
            <div className={`mt-3 pt-3 border-t ${theme === 'futuristic' ? 'border-cyan-500/30' : 'border-gray-300'}`}>
              <div className={`text-sm space-y-1 ${theme === 'futuristic' ? 'text-cyan-400' : 'text-gray-600'}`}>
        ...
        <div className="w-full flex flex-col items-end justify-center gap-4">
          ...
          <div className={`pointer-events-auto p-3 rounded-md transition-all ${theme === 'futuristic' ? 'neon-border bg-black/30 backdrop-blur-sm' : 'bg-gray-800 border border-gray-600'}`}>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs font-medium ${theme === 'futuristic' ? 'text-cyan-300' : 'text-white'}`}>
              ...
              {backendStatus === 'down' && (
                <button
                  ...
                  className={`ml-2 px-2 py-1 text-xs rounded transition-all ${theme === 'futuristic' ? 'neon-button-small text-cyan-300 hover:text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
          ...
          <button
            onClick={toggleTheme}
            className={`pointer-events-auto p-4 rounded-md transition-all ${theme === 'futuristic' ? 'neon-button accent pulse-glow' : 'bg-purple-500 hover:bg-purple-600 text-white'}`}
            title="Toggle Theme"
          >
            {theme === 'futuristic' ? (
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
